...
def test_process_input_reset_meta_skips_finalize_after_state_reset() -> None:
    route = TurnRoute(handler_name="meta", turn_mode="meta")
    raec = _build_lightweight_raec(intent=Intent.META, route=route, compress_result=False)

    finalize_calls: list[tuple[str, str, str, str]] = []

    def _finalize_turn(user_input: str, response: str, turn_mode: str, task_type: str) -> None:
        finalize_calls.append((user_input, response, turn_mode, task_type))

    raec._finalize_turn = _finalize_turn
    raec._handle_meta = lambda user_input, state_context="": "Conversation context reset for this session."

    response = raec.process_input("/reset", mode="auto")

    assert response == "Conversation context reset for this session."
